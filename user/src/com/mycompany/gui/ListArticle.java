...
public class ListArticle extends Form {
     ...
     public ListArticle(Form previous) {
        ...
        Display.getInstance().scheduleBackgroundTask(() -> {
            ...
            Display.getInstance().callSerially(() -> {
                ...
                list1 = ArticleService.getInstance().affichageArticle();
                ...
                for (Article article : list1) {
                    Label b = new Label(article.getDescription());

                    hi.add(b);

                    b.addPointerPressedListener(e -> {

                        if (article.getDescription() == b.getText()) {
                            new Search(previous).show();
                        }
                    });
                }
                ...
                List<Article> listeArticle = sp.affichageArticle();
                for (Article p : listeArticle) {
    ...
    btnModifier.addActionListener(e -> {
        new modifierArticle(this, p).show();
    });
                    ...
                    btnSupprimer.addActionListener(e -> {
                        ...
                        if (dig.show("Suppression", "Êtes-vous sûr de vouloir supprimer cet élément ?", "Annuler", "Oui")) {
                            ...
                            dig.dispose();
                            if (ArticleService.getInstance().deleteArticle(p.getId())) {
